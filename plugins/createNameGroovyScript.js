...
`;
  ...
  function getIntegrationFlowId(pluginHelper) {
    if (
      pluginHelper.currentArtifactType &&
      pluginHelper.currentArtifactType !== "IFlow"
    ) {
      return null;
    }

    return (
      pluginHelper.currentIflowId ||
      pluginHelper.currentArtifactId ||
  ...
  function normalizeFileName(value) {
    ...
    const nameWithExtension = name.toLowerCase().endsWith(".groovy")
      ? name
      : `${name}.groovy`;
    ...
    if (
      ...
      throw new Error(
        "Use a Groovy file name starting with a letter or underscore. Only letters, numbers, dot, dash and underscore are allowed.",
      );
    }
  ...
  async function resolveWorkspaceContext(pluginHelper, integrationFlowId) {
    ...
    if (!packageId) {
      throw new Error(
        "Open the integration flow from its integration package before creating a file.",
      );
    }

    const workspaceRoot = getWorkspaceApiRoot(pluginHelper);
    const workspacesResponse = await makeCallPromiseV2(
      "GET",
      `${workspaceRoot}/`,
      false,
      "application/json",
    ...
    if (!workspacesResponse.successful) {
      throwRequestError(
        workspacesResponse,
        "Could not read the integration packages.",
        `${workspaceRoot}/`,
      );
    }

    const workspaces = parseJsonResponse(
      workspacesResponse,
      "SAP returned an unreadable integration package list.",
      `${workspaceRoot}/`,
    );
    const workspace = Array.isArray(workspaces)
      ? workspaces.find((item) => item.technicalName === packageId)
      : null;
    if (!workspace?.id) {
      throw new Error(
        `The integration package '${packageId}' was not found in the current workspace.`,
      );
    }

    const artifactsUrl = `${workspaceRoot}/${encodePathSegment(workspace.id)}/artifacts/`;
    const artifactsResponse = await makeCallPromiseV2(
      "GET",
      artifactsUrl,
      false,
      "application/json",
    ...
    if (!artifactsResponse.successful) {
      throwRequestError(
        artifactsResponse,
        "Could not read the integration package artifacts.",
        artifactsUrl,
      );
    }

    const artifacts = parseJsonResponse(
      artifactsResponse,
      "SAP returned an unreadable artifact list.",
      artifactsUrl,
    );
    const artifact = Array.isArray(artifacts)
      ? artifacts.find((item) => item.tooltip === integrationFlowId)
      : null;
    const entityId = artifact?.entityID || artifact?.id;
    if (!entityId) {
      throw new Error(
        `The integration flow '${integrationFlowId}' was not found in package '${packageId}'.`,
      );
    }
  ...
  async function ensureScriptDoesNotExist(
    ...
    if (!response.successful) {
      throwRequestError(
        response,
        "Could not read the existing Groovy files.",
        scriptsUrl,
      );
    }
    ...
    const scripts = parseJsonResponse(
      response,
      "SAP returned an unreadable Groovy file list.",
      scriptsUrl,
    );
    if (!Array.isArray(scripts)) {
      throw new Error(
        `SAP returned an unexpected Groovy file list. Request: ${scriptsUrl}`,
      );
    }

    const requestedBaseName = fileName
      .slice(0, -".groovy".length)
      .toLowerCase();
    const alreadyExists = scripts.some(
      (script) =>
        String(script.fileName || "").toLowerCase() === requestedBaseName,
    );
    if (alreadyExists) {
  ...
  async function createGroovyResource(
    pluginHelper,
    integrationFlowId,
    fileName,
  ) {
    const { entityBaseUrl } = await resolveWorkspaceContext(
      pluginHelper,
      integrationFlowId,
    );
    ...
    const createResponse = await makeCallPromiseV2(
      "PUT",
      scriptUrl,
      false,
      "application/json",
      JSON.stringify({ content: DEFAULT_GROOVY_TEMPLATE }),
      true,
      "application/json",
      true,
    );
    if (!createResponse.successful) {
      throwRequestError(
        createResponse,
        "SAP did not create the Groovy file.",
        scriptUrl,
      );
    }
    ...
    const draftResponse = await makeCallPromiseV2(
      "POST",
      draftUrl,
      false,
      "application/json",
    ...
    if (!draftResponse.successful) {
      throwRequestError(
        draftResponse,
        "The Groovy file was created, but SAP could not save its draft.",
        draftUrl,
      );
    }
  }
  ...
  function createDialog(pluginHelper) {
    ...
    help.textContent =
      "The new file contains a minimal processData(Message message) template.";
    ...
    const submit = async () => {
      try {
        ...
        setStatus(
          status,
          `Created '${fileName}'. Refresh the iFlow resources to display the new file.`,
          "positive",
        );
        ...
        setStatus(
          status,
          error.message || "Could not create the Groovy resource.",
          "negative",
        );
  ...
  const plugin = {
    ...
    version: "0.1.0",
    author: "Bjoern / local development",
    website: "https://github.com/dbeck121/CPI-Helper-Chrome-Extension",
    email: "",
    description:
      "Adds a Create File button to the message sidebar. It creates a named Groovy resource in the current integration flow.",
    settings: {},
    messageSidebarContent: {
      onRender: (pluginHelper) => {
        ...
        button.title =
          "Create a named Groovy file in the current integration flow";
