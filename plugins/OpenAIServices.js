var plugin = {
    ...
    messageSidebarContent: {
        ...
        onRender: (pluginHelper, settings) => {
            var strToUnit8ArrayFunction = function stringToUint8Array(
                s,
                length
            ) {
            ...
            div.querySelector("#mode").addEventListener("click", () => {
                ...
                if (mode == "edit") {
                    div.querySelector("#instructionRow").removeAttribute(
                        "hidden"
                    );
                } else {
                    div.querySelector("#instructionRow").setAttribute(
                        "hidden",
                        "hidden"
                    );
                }
            ...
            div.querySelector("#submit").addEventListener("click", () => {
                ...
                if (mode == "insert" && request.indexOf("[insert]") == -1) {
                    alert(
                        "Use [insert] to indicate where the text or code should added."
                    );
                    return;
                }
                ...
                if (usedToken > maxToken) {
                    alert(
                        "Request is too long. Please limit to " +
                            maxToken +
                            " tokens."
                    );
                    return;
            ...
            div.querySelector("#model").addEventListener("click", () => {
                ...
                if (selectedModel) {
                    ...
                    let maxRequestTokens =
                        models_list[selectedModel].maxRequestTokens;
            ...
            div.querySelector("#request").addEventListener("input", () => {
                try {
                    ...
                    div.querySelector("#token").innerHTML =
                        token + " tokens approximately.";
                } catch (exp) {
                    alert(exp);
                }
            });
            div.querySelector("#set").addEventListener("click", async () => {
                ...
                if (apiKey != null) {
                    let keyPass = prompt(
                        "Please enter a password to encrypt your key."
                    );
                    if (keyPass != null) {
                        try {
                            let keyMaterial =
                            ...
                            await syncChromeStoragePromise(
                                getStoragePath("OpenAI", "APIKey"),
                                btoa(new Uint8Array(encryptedAPIKey))
                            );

                            showToast(
                                "API key encrypted and save successfully!"
                            );
            ...
            div.querySelector("#load").addEventListener("click", async () => {
                let keyPass = prompt(
                    "Please enter password to retrieve your key."
                );
                if (keyPass != null) {
                    try {
                        let encryptedKey = await getStorageValue(
                            "OpenAI",
                            "APIKey"
                        );
                        ...
                        if (encryptedKey != "") {
                            ...
                            let decryptedKey =
                                await window.crypto.subtle.decrypt(
                                    ...
                                    secretKey,
                                    Uint8Array.from(
                                        atob(encryptedKey).split(","),
                                        (c) => c
                                    )
                                );
                            div.querySelector("#apiKey").value =
                                dec.decode(decryptedKey);
            ...
            actionButton.onclick = (x) =>
                pluginHelper.functions.popup(div, "My AI Coding Buddy");
