...
var plugin = {
    ...
    messageSidebarContent: {
        ...
        onRender: (pluginHelper, settings) => {
            ...
            const inputSteps = inputtrace.querySelector(
                `#traceModifer_${pluginHelper.currentArtifactId}`
            );
            ...
            inputSteps.addEventListener("change", () => {
                ...
                if ((data === null && data === undefined) || data === "") {
                    ...
                    chrome.storage.local.set(
                        JSON.parse(
                            `{ "traceModifer_${pluginHelper.currentArtifactId}":"${data}"}`
                        )
                    );
                }
                log.debug("set", data);
        ...
        onClick: async (pluginHelper, settings, runInfo, active) => {
            if (active) {
                ...
                if (inlineTrace) {
                    ...
                    showToast(
                        "No data found.",
                        "Performance Stats not possible",
                        "warning"
                    );
                }
            } else {
