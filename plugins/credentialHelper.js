    ...
    messageSidebarContent: {
        ...
        onRender: (pluginHelper) => {
            ...
            function handleTyping(event) {
                ...
                const dropdownMaxHeight = parseInt(
                    dropdown.style.maxHeight.replace("px", ""),
                    10
                );
                const dropdownHeight = Math.min(
                    dropdownMaxHeight,
                    matchingAliases.length * 40
                ); // Estimate each item height
                    ...
                    if (hasMatches == true) {
                        ...
                        item.addEventListener("click", () => {
                            ...
                            const matchedPattern = patterns.find((pattern) =>
                                item.dataset.originInput.includes(pattern)
                            );
                            const selector = patterns
                                .map(
                                    (pattern) =>
                                        `input[id*="__input_"][id*="${matchedPattern}"]`
                                )
                                .join(", ");
            ...
            document.addEventListener("click", async function (event) {
                ...
                if (
                    event.target?.attributes?.readonly?.textContent !=
                        "readonly" &&
                    event.target.tagName === "INPUT" &&
                    event.target.id.includes("__input_") &&
                    patterns.some((pattern) =>
                        event.target.id.includes(pattern)
                    )
                ) {
                    ...
                    var keyStoreEntries = JSON.parse(
                        await makeCallPromise(
                            "GET",
                            urlForKeyStoreEntries,
                            false,
                            "application/json"
                        )
                    ).d.results.map((ks) => ks.Alias);
                    ...
                    securityMaterialList = JSON.parse(
                        await makeCallPromise(
                            "GET",
                            urlForSecurityMaterialList,
                            false,
                            "application/json"
                        )
                    ).artifactInformations.map((sm) => sm.name);
            ...
            document.addEventListener("click", function (event) {
                ...
                const isDropdownItem = event.target.classList.contains(
                    "credentialHelperDropdown-item"
                );
                const isInputField =
                    event.target.tagName === "INPUT" &&
