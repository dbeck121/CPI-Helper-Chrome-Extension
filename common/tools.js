        ...
        return new Promise(async function (resolve, reject) {
            ...
            xhr.onload = function () {
                if (this.status >= 200 && this.status < 300) {
                    ...
                    showInfo
                        ? showToast(
                              "CPI-Helper has run into a problem while catching X-CSRF-Token.",
                              "",
                              "error"
                          )
                        : {};
            ...
            xhr.ontimeout = function () {
                ...
                showInfo
                    ? showToast(
                          "CPI-Helper has run into a timeout while refreshing X-CSRF-Token.",
 ...
 */
async function makeCallPromiseXHR(
    method,
    url,
    accept,
    payload,
    ...
    return new Promise(async function (resolve, reject) {
        ...
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                ...
                showInfo
                    ? showToast(
                          "CPI-Helper has run into a problem while loading data.",
                          "",
                          "error"
                      )
                    : {};
        ...
        xhr.ontimeout = function (e) {
            ...
            showInfo
                ? showToast(
                      "CPI-Helper has run into a timeout",
                      "Please refresh page and try again.",
                      "error"
                  )
                : {};
...
async function makeCallPromise(
    method,
    url,
    useCache,
    accept,
...
var formatTrace = function (input, id, traceId) {
    ...
    var formatXml = function (sourceXml, tab_size) {
        var xmlDeclarationmatch = sourceXml.match(
            /^<\?xml\s+version\s*=\s*(["'])[^\1]+\1\s+encoding\s*=\s*(["'])[^\2]+\2\s*\?>/
        );
        var xmlDeclaration = xmlDeclarationmatch
        ...
        if (xmlDoc.getElementsByTagName("parsererror").length > 0) {
            var xmlDoc = new DOMParser().parseFromString(
                `<cpi_Helper>${sourceXml}</cpi_Helper>`,
                "application/xml"
            );
            filterflag = 1;
        }
        ...
        if (filterflag === 1) {
            resultXml = resultXml
                .substring(12, resultXml.length - 13)
                .replaceAll("\n  ", "\n");
        }
    ...
    var prettify_type = function (input) {
        ...
        let sqloccurence = input
            ...
            .toLowerCase()
            .match(
                /select|from|where|update|insert|upsert|create table|union|join|values|group by/gm
            )?.length;
        if (
            (sqloccurence && sqloccurence >= 2) ||
            (input.substring(0, 2).match("--")?.length === 2 &&
                sqloccurence >= 1) ||
            input.substring(0, 6).match("--sql")
        ) {
            return "sql";
        }
        return "text";
        ...
        downloadButton.onclick = async (element) => {
            var response = await makeCallPromise(
                ...
                "/" +
                    ...
                    "Operations/com.sap.it.op.tmn.commands.dashboard.webui.GetTraceArchiveCommand?traceIds=" +
                    traceId,
                true
            );
    ...
    copyButton.onclick = (input) => {
        var text;
        //check who is active
        var unformatted = document.getElementById(
            "cpiHelper_traceText_unformatted_" + id
        );
    ...
    themeButton.innerText = `${
        $("#cpihelperglobal").hasClass("ch_dark") ? "Dark" : "Light"
    } Editor`;
    themeButton.onclick = (event) => {
        themeButton.innerText = `${
            editorManager.toggleTheme() ? "Dark" : "Light"
        } Editor`;
    };
    ...
    readonlyButton.onclick = (event) => {
        readonlyButton.innerText = editorManager.toggleReadOnly()
            ? "Edit"
            : "Read Only";
    };
    ...
    beautifyButton.onclick = (event) => {
        ...
        if ($formatted.text().trim() === "") {
            editorManager = new EditorManager(
                "cpiHelper_traceText_formatted_" + id,
                prettify_type(input),
                $("#cpihelperglobal").hasClass("ch_dark")
                    ? "github_dark"
                    : "textmate"
            );
    ...
    if (!input) {
        result.innerHTML =
            '<div class="cpiHelper_infoPopUp_content">No elements found. If this should be part of the trace of an adapter step, try other tabs with same step Id on top of this popup.</div>';
        return result;
    }
    ...
    if (textEncoder.length) {
        ...
        if (kb > 2000) {
            additionalText +=
                " - large payload. Beautify could take a while. Browser might freeze.";
        }
        if (kb > 25000) {
            additionalText +=
                " - maybe original payload is larger but we can't show it here and load more";
        }

        span.innerText =
            " Length unformated: " +
            input.split(/\r\n|\r|\n/).length +
            " lines; Size unformated: " +
            textEncoder.length +
            " bytes, " +
            kb +
            " KB, " +
            Math.round((kb / 1024) * 100) / 100 +
            " MB" +
            additionalText;
