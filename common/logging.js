...
const levelMap = Object.entries(ulog.levels).reduce(
...
log.log(
    "Checking for timeout in url parameter cpihelper_debug_download_duration that triggers log download"
);
if (window.location.href.indexOf("cpihelper_debug_download_duration=") > -1) {
    timeout_string = window.location.href
        .split("cpihelper_debug_download_duration=")[1]
        .split("&")[0];
...
function adjustLogLevelByTime(timeout = $("#timeout").val()) {
    ...
    if (timerId) {
        ...
        log.log(
            String(levelMap[log.level]) + " mode active " + timeout + " ms"
        );
        showToast(
            String(levelMap[log.level]) + " is activated for " + timeout + " ms"
        );
