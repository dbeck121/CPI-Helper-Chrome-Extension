...
function showToast(message, title, type = "") {
    ...
    $.toast({
        class:
            type +
            ($("html").hasClass("sapUiTheme-sap_horizon_dark")
                ? " ch_dark "
                : ""),
...
async function showBigPopup(
    content,
    header,
    parameters = { fullscreen: true, callback: null, onclose: null },
    count = 0,
    maxcount = 0,
    type = "mouse"
) {
    ...
    if ($modal.length) {
        ...
        ["negative", "positive"].forEach((type, index) => {
            ...
            if ($button.length) {
                $button.on("click", () => {
                    const sortedArray = $(
                        ".cpiHelper_onclick[inline_cpi_child]"
                    )
                        .map((_, e) =>
                            parseInt($(e).attr("inline_cpi_child"), 10)
                        )
                        .get()
                        .sort((a, b) => a - b);
                    console.log(
                        sortedArray,
                        $("#cpiHelper_semanticui_modal .header").attr("count"),
                        sortedArray[
                            $("#cpiHelper_semanticui_modal .header").attr(
                                "count"
                            )
                        ],
                    ...
                    if (
                        ...
                        $(
                            `[inline_cpi_child=${element}] .cpiHelper_inlineInfo`
                        ).trigger("click");
                        showToast(
                            `${
                                index ? "Next" : "Previous"
                        ...
                        showToast(
                            `${index ? "Next" : "Previous"} Step is not found`,
        ...
        $modal
            ...
            .modal({
                ...
                onHidden: function () {
                    if (
                        parameters.onclose &&
                        parameters.onclose instanceof Function
                    ) {
                        parameters.onclose();
                    }
                }
            ...
            if (typeof objects[i].content == "function") {
                input.onclick = async (event) => {
                    ...
                    if (
                        ...
                        let contentResponse = await objects[i].content(
                            objects[i]
                        );
