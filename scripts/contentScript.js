...
cpiArtifactURIRegexp = [
    ...
    //resources
    [
        /\/resources\/mapping\/(?<artifactId>[0-9a-zA-Z_\-.]+\.mmap?)/,
        "M_Mapping"
    ],
    [
        /\/resources\/mapping\/(?<artifactId>[0-9a-zA-Z_\-.]+\.opmap?)/,
        "Operation Mapping"
    ],
...
var cpiCollectionURIRegexp =
    /\/contentpackage\/(?<artifactId>[0-9a-zA-Z_\-.]+)/;
...
async function renderMessageSidebar() {
    ...
    await getIflowInfo((data) => {
        ...
        if (deploymentText) {
            let statusColor = getStatusColorCode(
                cpiData?.flowData?.artifactInformation?.deployState
            );
    ...
    if (xhr.readyState == 4 && sidebar.active) {
        ...
        try {
            ...
            log.error(
                "There was a faulty message from CI-API. CPI Helper will ignore it: " +
                    e
            );
        }
        ...
        if (resp && resp.length != 0) {
            ...
            try {
                if (
                    thisMessageHash !=
                    cpiData.messageSidebar.lastMessageHashList[0]
                ) {
                    ...
                    for (var i = 0; i < count; i++) {
                        ...
                        runInfoElement[thisMessageHash].messageHash =
                            resp[i].MessageGuid +
                            resp[i].LogStart +
                            resp[i].LogEnd +
                            resp[i].Status;
                        runInfoElement[thisMessageHash].messageGuid =
                            resp[i].MessageGuid;
                        runInfoElement[thisMessageHash].logStart = new Date(
                            parseInt(resp[i].LogStart.match(/\d+/)[0])
                        );
                        runInfoElement[thisMessageHash].logEnd = new Date(
                            parseInt(resp[i].LogEnd.match(/\d+/)[0])
                        );
                        runInfoElement[thisMessageHash].status = resp[i].Status;
                        runInfoElement[thisMessageHash].message =
                            resp[i].LogLevel;
                        ...
                        runInfoElement[thisMessageHash].timeZoneOffset =
                            date.getTimezoneOffset();
                        date = date.toISOString();

                        if (date.substr(0, 10) != lastDay) {
                            messageList.appendChild(
                                createRow([date.substr(0, 10)])
                            );
                        ...
                        let flash = "";
                        if (
                            cpiData.messageSidebar.lastMessageHashList.length !=
                                0 &&
                            !cpiData.messageSidebar.lastMessageHashList.includes(
                                thisMessageHashList[i]
                            )
                        ) {
                            flash = " flash";
                        }
                        ...
                        let traceButton = createElementFromHTML(
                            `<button title='jump to trace page' id='trace--${i}' class='${
                                resp[i].MessageGuid
                            } ${flash}'>${loglevel
                                .substr(0, 1)
                                .toUpperCase()}</button>`
                        ...
                        activeInlineItem ==
                            quickInlineTraceButton.classList[0] &&
                            quickInlineTraceButton.classList.add(
                        ...
                        let statusicon = createElementFromHTML(
                            `<button title='Status Details' class='cpiHelper_inlineInfo-button'><span data-sap-ui-icon-content='&#${statusIcon}' class='${resp[i].MessageGuid}` +
                                ...
                                `${statusColor}'></span>` +
                                //timeButton here
                                `<span style='color:${statusColor};padding-inline-start:0.3em'>${date.substr(
                                    11,
                                    8
                                )}</span></button>`
                        );
                        ...
                        quickInlineTraceButton.onmouseup = async (e) => {
                            ...
                            if (
                                ...
                                hideInlineTrace();
                                var inlineTrace = await showInlineTrace(
                                    e.currentTarget.parentNode.parentNode
                                        .className
                                );
                                if (inlineTrace) {
                                    statistic(
                                        "messagebar_btn_inlinetrace_click"
                                    );
                                    showToast("Inline-Debugging Activated");
                                    mytarget.classList.add(
                                        "cpiHelper_inlineInfo-active"
                                    );
                                    activeInlineItem =
                                        mytarget.parentNode.parentNode
                                            .className;
                                } else {
                                    activeInlineItem = null;
                                    showToast(
                                        "No data found.",
                                        "Inline debugging not possible",
                                        "warning"
                                    );
                                }
                            }
                        ...
                        var pluginButtons =
                            await createPluginButtonsInMessageSidebar(
                                runInfoElement[thisMessageHash],
                                i,
                                flash
                            );
                        ...
                        logButton.addEventListener("click", async (a) => {
                            ...
                            await showBigPopup(
                                await createContentNodeForLogs(
                                    a.currentTarget.classList[0],
                                    false
                                ),
                                "Logs"
                            );
                ...
                log.error(
                    "There was an error when processing the log entries. Process aborted. " +
                        e
                );
            }
        }
...
function calculateMessageSidebarTimerTime(lastTabHidden, lastDurationRefresh) {
    ...
    if (lastDurationRefresh > 2000) {
        log.debug(
            "Last rendering took more than 2000ms, set timer to 18 seconds"
        );
    ...
    if (lastDurationRefresh > 1000) {
        log.debug(
            "Last rendering took more than 1000ms, set timer to 9 seconds"
        );
    ...
    if (lastDurationRefresh > 500) {
        log.debug(
            "Last rendering took more than 500ms, set timer to 6 seconds"
        );
        messageSidebarTimerTime = 2;
        return messageSidebarTimerTime;
    }
...
async function clickTrace(e) {
    ...
    var formatInfoContent = function (inputList) {
        ...
        stepStart.setTime(
            stepStart.getTime() - stepStart.getTimezoneOffset() * 60 * 1000
        );
        ...
        if (inputList.StepStop) {
            ...
            stepStop.setTime(
                stepStop.getTime() - stepStop.getTimezoneOffset() * 60 * 1000
            );
            valueList.push({
                Name: "End Time",
    ...
    var getTraceTabContent = async function (object) {
        var traceData = JSON.parse(
            await makeCallPromise(
                ...
                "/" +
                    cpiData.urlExtension +
                    "odata/api/v1/MessageProcessingLogRunSteps(RunId='" +
                    object.runId +
                    "',ChildCount=" +
                    object.childCount +
                    ")/TraceMessages?$format=json",
                true
            )
        ).d.results;
        ...
        if (!trace) {
            showToast(
                "it is already deleted or not in trace mode.",
                "No trace exists",
                "warning"
            );
        ...
        if (object.traceType == "properties") {
            let elements = JSON.parse(
                await makeCallPromise(
                    ...
                    "/" +
                        cpiData.urlExtension +
                        "odata/api/v1/TraceMessages(" +
                        traceId +
                        ")/ExchangeProperties?$format=json",
                    true
                )
            ).d.results;
        ...
        if (object.traceType == "trace") {
            let elements = await makeCallPromise(
                "GET",
                "/" +
                    cpiData.urlExtension +
                    "odata/api/v1/TraceMessages(" +
            ...
            html = formatTrace(
                elements,
                object.runId + "_" + object.childCount,
                traceId
            );
        }
            ...
            let elements = JSON.parse(
                await makeCallPromise(
                    ...
                    "/" +
                        ...
                        "',ChildCount=" +
                        object.childCount +
                        ")/?$expand=RunStepProperties&$format=json",
                    true
                )
    ...
    var messageguid = document
        .querySelector(
            ".cpiHelper_inlineInfo-button.cpiHelper_inlineInfo-active"
        )
        .className.replace("flash", "")
        .replace(/cpiHelper_inlineInfo-[A-z]+|\s+/g, "")
        .trim();
    var logleveldata = JSON.parse(
        await makeCallPromise(
            "GET",
    ...
    if (logleveldata.LogLevel != "TRACE") {
        ...
        showToast(
            ...
            "your log level is" + logleveldata.LogLevel,
            "warning"
        );
    } else if (
        logleveldata.LogLevel == "TRACE" &&
        new Date(
            parseInt(logleveldata.LogEnd.replace(/\D/g, "")) + 1.05 * 60 * 60000
        ) < new Date()
    ) {
        $("#cpiHelper_waiting_model").modal("hide");
        ...
        async function loginformation() {
            {
                ...
                for (var n = targetElements.length - 1; n >= 0; n--) {
                    ...
                    try {
                        ...
                        let label = "" + branch;
                        let content = await createTabHTML(
                            objects,
                            "tracetab-" + childCount
                        );
                        if (content) {
                ...
                return runs.length == 1
                    ? runs[0].content
                    : await createTabHTML(runs, "runstab", 0);
            }
        }
        let childindex = Array.from(
            document.querySelectorAll(".cpiHelper_onclick[inline_cpi_child]"),
            (e) => parseInt(e.getAttribute("inline_cpi_child"), 10)
        ).sort((a, b) => a - b);
        //console.log(e.target.parentNode.parentNode)
        childindex = childindex.indexOf(
            parseInt(
                e.target.parentNode.parentNode.getAttribute("inline_cpi_child")
            )
        );
        showBigPopup(
            await loginformation,
            "Content Before Step",
            { fullscreen: true, callback: null },
            childindex,
            document.querySelectorAll(".cpiHelper_onclick[inline_cpi_child]")
                .length,
            String(e.pointerType)
        );
    }
...
function timenodediff(e) {
    return {
        ...
        CH_stats:
            parseInt(e.StepStop.match(/\d+/)[0]) -
            parseInt(e.StepStart.match(/\d+/)[0])
    };
}
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (await getStorageValue("traceModifer", "isActive", null)) {
            if (Array.isArray(logRuns) && checked) {
                ...
                cpi_nodes = logRuns
                    .filter((e) => {
                        return e.StepStart != null && e.StepStop != null
                            ? true
                            : false;
                    })
                    .map(timenodediff);
...
async function showInlineTrace(MessageGuid, checked = false) {
    return new Promise(async (resolve, reject) => {
        ...
        var Trace_bool = await getStorageValue(
            "traceModifer",
            "isActive",
            null
        );
        ...
        inlineTraceElements.forEach((run) => {
            try {
                ...
                if (/ServiceTask/.test(run.StepId)) {
                    ...
                    target =
                        element.children[getChild(element, ["g"])].children[0];
                }
                ...
                if (
                    /MessageFlow_\d+/.test(run.ModelStepId) &&
                    /#/.test(run.ModelStepId) != true
                ) {
                    element = document.getElementById(
                        "BPMNEdge_" + run.ModelStepId
                    );
                    target =
                        element.children[
                            getChild(element, ["text"], "shapeText")
                        ];
                }
                ...
                if (Trace_bool && checked) {
                    indexofnode = cpi_timediff_list.findIndex(
                        (e) =>
                            e ===
                            cpi_max_node.find(
                                (f) => f.ModelStepId === run.ModelStepId
                            ).CH_stats
                    );
                    if (indexofnode == cpi_timediff_list.length - 1) {
                        ...
                        indexofnode ==
                        (cpi_timediff_list.length % 2 === 0
                            ? cpi_timediff_list.length / 2
                            : Math.round(cpi_timediff_list.length / 2) - 1)
                    ) {
...
function addBreadcrumbs() {
    const crumbs = $('nav[id*="breadcrumbs"]')
        .find("ol:first-child")
        .find("li");
    if (crumbs) {
        if (crumbs.length == 1) {
            ...
            if ((regexMatch = regex.exec(url)) !== null) {
                packageUrl =
                    regexMatch[1] + regexMatch[2] + "?section=ARTIFACTS";
...
async function buildButtonBar() {
    ...
    if (!document.getElementById("__buttonxx")) {
        ...
        area = document.querySelector(
            "[id*='--iflowObjectPageHeader-actions']"
        );
        ...
        if (area) {
            ...
            area.addEventListener("click", () => {
                document.querySelector("[id*='--searchStep-I']").accessKey =
                    "s";
            });
        ...
        log.debug(
            "Artifect from checks for sidebar",
            cpiData.currentArtifactType
        );
        if (
            (sidebar.active == null || sidebar.active == false) &&
            cpiData.currentArtifactType
        ) {
            chrome.storage.sync.get(
                ["openMessageSidebarOnStartup"],
                function (result) {
                    ...
                    if (result["openMessageSidebarOnStartup"] === undefined) {
                        ...
                        openMessageSidebarOnStartupValue =
                            result["openMessageSidebarOnStartup"];
                    }
...
async function getIflowInfo(callback, silent = false) {
    return makeCallPromise(
        ...
        .then((response) => {
            response = new XmlToJson().parse(response)[
                "com.sap.it.op.tmn.commands.dashboard.webui.IntegrationComponentsListResponse"
            ];
            ...
            return makeCallPromise(
                ...
                "/" +
                    cpiData.urlExtension +
                    "Operations/com.sap.it.op.tmn.commands.dashboard.webui.IntegrationComponentDetailCommand?artifactId=" +
                    resp.id,
                false,
                "application/json",
                null,
      ...
      <div>DeployedBy: ${cpiData?.flowData?.artifactInformation?.deployedBy}</div>
        ...
        if (
            ...
            cpiData.flowData.endpointInformation.forEach((element) => {
                if (
                    element.endpointInstances &&
                    element.endpointInstances.length > 0
                ) {
                    ...
                    for (var i = 0; i < element.endpointInstances.length; i++) {
                        ...
                        quickCopyToClipboardButton.onclick = (event) => {
                            copyText(
                                event.srcElement.getAttribute("data-text")
                            );
                        };
        ...
        async function createTableForVariables() {
            var variableList = await makeCallPromise(
                ...
                "/" +
                    cpiData.urlExtension +
                    "Operations/com.sap.esb.monitoring.datastore.access.command.ListDataStoreEntriesCommand?storeName=sap_global_store&allStores=true&maxNum=100000",
                false,
                "application/json",
                null,
                false
            ...
            variableList = variableList.filter(
                (element) =>
                    !element.qualifier ||
                    element.qualifier ==
                        cpiData?.flowData?.artifactInformation?.symbolicName
            );
            ...
            variableList.forEach((item) => {
                ...
                downloadButton.onclick = async (element) => {
                    ...
                    var response = await makeCallPromise(
                        ...
                        "/" +
                            cpiData.urlExtension +
                            "Operations/com.sap.esb.monitoring.datastore.access.command.GetDataStorePayloadCommand",
                        false,
                        "",
                ...
                showButton.onclick = async (element) => {
                    text = document.getElementById(
                        item.id + item.storeName + "_value"
                    );

                    if (
                        text.classList.contains("cpiHelper_infoPopUp_TR_hide")
                    ) {
                        try {
                            let payload = {
                            ...
                            var response = await makeCallPromise(
                                ...
                                "/" +
                                    cpiData.urlExtension +
                                    "Operations/com.sap.esb.monitoring.datastore.access.command.GetDataStoreVariableCommand",
                                false,
                                "",
                                JSON.stringify(payload),
                                true,
                            ...
                            var value =
                                response.match(/<value>(.*)<\/value>/gs)[0];
                            ...
                            if (agressiveMode) {
                                showToast(
                                    "Aggressive mode was used to show variable"
                                );
                            }
                            ...
                            showToast(
                                ...
                                "Please download and try manually."
                            );
                        }
                    } else {
                ...
                deleteButton.onclick = async (element) => {
                    var doDelete = getConfirmation(
                        `Do you really want to delete variable \"${item.id}\"? You can not undo this.`
                    );
                    if (doDelete) {
                        //delete Variable
                        try {
                            ...
                            var response = await makeCallPromise(
                                ...
                                "/" +
                                    cpiData.urlExtension +
                                    "Operations/com.sap.esb.monitoring.datastore.access.command.DeleteDataStoreEntryCommand",
                                false,
                                "",
                                JSON.stringify(payload),
                                true,
                            ...
                            let cpiHelper_infoPopUp_Variables =
                                document.getElementById(
                                    "cpiHelper_infoPopUp_Variables"
                                );
        ...
        if (deployedOn) {
            ...
            undeploybutton.addEventListener("click", (a) => {
                undeploy(
                    cpiData?.flowData?.artifactInformation?.tenantId,
...
var sidebar = {
    ...
    `;
        ...
        chrome.storage.sync.get(["openSidebarOnStartup"], function (result) {
            pluginarea = document.querySelector(
                "#cpiHelper_messageSidebar_pluginArea"
            );
            if (result["openSidebarOnStartup"]) {
                pluginarea.classList.add("sidebar");
                pluginarea.classList.toggle("fluid");
                document
                    .querySelector("#cpiHelper_messageSidebar_pluginArea span")
                    .addEventListener("click", () => {
                        pluginarea.classList.toggle("fluid");
                        twoClasssToggleSwitch(
                            pluginarea,
                            "visible",
                            "cpiHelper_hidden"
                        );
                        twoClasssToggleSwitch(
                            document.querySelector("#sidebar_Plugin"),
                            "plus",
                            "minus"
                        );
                    });
        ...
        var borderofouterFrame =
            getComputedStyle(outerFrame_element).borderRadius.split(" ");
        span.onclick = () => {
            if (outerFrame_element.offsetHeight > 0) {
                content_header.style["min-width"] =
                    getComputedStyle(outerFrame_element).width;
                outerFrame_element.style.display = "none";
                content_header.style["border-bottom-left-radius"] =
                    borderofouterFrame[2];
                content_header.style["border-bottom-right-radius"] =
                    borderofouterFrame[3];
            } else {
                outerFrame_element.style.display = "block";
                content_header.style["border-bottom-left-radius"] =
                    borderofouterFrame[0];
                content_header.style["border-bottom-right-radius"] =
                    borderofouterFrame[1];
            }
        };
        ...
        setTimeout(
            () =>
                document
                    .getElementById("cpiHelper_content")
...
async function errorPopupOpen(MessageGuid) {
    ...
    var customHeaders = await makeCallPromise(
        "GET",
        "/" +
            cpiData.urlExtension +
            "odata/api/v1/MessageProcessingLogs('" +
            MessageGuid +
            "')?$format=json&$expand=CustomHeaderProperties",
        false
    );
    ...
    propertyArray = [];
    customHeaders?.CustomHeaderProperties?.results.forEach((element) =>
        propertyArray.push(
            element?.Name + ": " + element?.Value?.substr(0, 150)
        )
    );
    // Error Collect
    errorDetails = [];
...
async function popupTable(message) {
  ...
  <tbody>
    <tr class="center aligned">
      ...
      <td class="${getStatusColor(data.status)}">${getStatusIcon(data.status)}${
        data.status
    }</td>
    </tr>
    <tr class="center aligned">
      <td class="info">Custom Status</td>
      <td class="${getStatusColor(data.customstatus)}">${getStatusIcon(
        data.customstatus
    )}${data.customstatus}</td>
    </tr>
    <tr class="center aligned">
    ...
    if (data.errors.length > 0 || data.property.length > 0) {
      ...
      <div class="ui fluid scrolling segment" style="text-wrap: pretty;width: unset;word-break: break-word;">`;
        ...
        if (data.errors.length > 0) {
            ...
            popupHTML += data.errors
                .map((e) => `<span class="ui red text">${e}</span>`)
...
function apireserror(message) {
    ...
    $.toast({
        ...
        showProgress: "bottom",
        class: $("html").hasClass("sapUiTheme-sap_horizon_dark")
            ? " ch_dark "
            : "",
        onVisible: async () =>
            popupTable(message)
...
function lookupError(message) {
    if (
        /unable to find valid certification path to requested target/.test(
            message
        )
    ) {
...
async function getMessageProcessingLogRuns(MessageGuid, store = true) {
    ...
    top_mode_count =
        top_mode_count == null && top_mode_count == undefined
            ? "&$top=300"
            : `& $top=${parseInt(top_mode_count)} `; //Default
    if (top_mode_count === "&$top=0") {
        top_mode_count = "";
    }
    //Plugin over-write
    if (await getStorageValue("traceModifer", "isActive", null)) {
        var top_mode_count_flow = await storageGetPromise(
            `traceModifer_${cpiData.integrationFlowId} `
        );
        console.debug(
            "traceModifer_flow",
            cpiData.integrationFlowId,
        ...
        top_mode_count =
            (top_mode_count_flow == null && top_mode_count_flow == undefined) ||
            top_mode_count_flow == 0
                ? top_mode_count
                : `& $top=${parseInt(top_mode_count_flow)} `;
    }
    ...
    return makeCallPromise(
        ...
        "/" +
            ...
            "odata/api/v1/MessageProcessingLogs('" +
            MessageGuid +
            "')/Runs?$inlinecount=allpages&$format=json&$top=200",
        store
    )
        .then((responseText) => {
        ...
        .then((runId) => {
            return makeCallPromise(
                ...
                "/" +
                    cpiData.urlExtension +
                    "odata/api/v1/MessageProcessingLogRuns('" +
                    runId +
                    "')/RunSteps?$inlinecount=allpages&$format=json" +
                    top_mode_count,
                store
            );
        })
        .then((response) => {
            return JSON.parse(response).d.results.filter(
                (e) => e.StepStop != null
            );
        })
        .catch((e) => {
    ...
    if (cpiData.currentArtifactType == "IFlow") {
        ...
        if (buttonsForPlugins.length > 0) {
            ...
            var interval = setInterval(() => {
                var pluginArea = document.querySelector(
                    'span[id$="--scriptPageContainerHeader-identifierLineContainer"]'
                );
                if (!pluginArea) {
                    pluginArea = document.querySelector(
                        'span[id$="--scriptPageHeaderTitle-identifierLineContainer"]'
                    );
                }
                if (
                    (pluginArea && scriptCount > 10) ||
                    cpiData.currentArtifactType != "Script"
                ) {
                ...
                buttons = document.getElementsByClassName(
                    "cpiHelper_pluginButton_scriptButton"
                );
        ...
        var buttonsForPlugins = await createPluginButtons(
        ...
        if (buttonsForPlugins.length > 0) {
            ...
            var interval = setInterval(() => {
                var pluginArea = document.querySelector(
                    'span[id$="--objectPageHeader-identifierLineContainer"]'
                );
                if (
                    (pluginArea && scriptCollectionCount > 10) ||
                    cpiData.currentArtifactType != "Script Collection"
                ) {
                    clearInterval(interval);
                ...
                buttons = document.getElementsByClassName(
                    "cpiHelper_pluginButton_scriptCollectionButton"
                );
                if (pluginArea && buttons.length == 0) {
        ...
        if (buttonsForPlugins.length > 0) {
            ...
            var interval = setInterval(() => {
                var pluginArea = document.querySelector(
                    'span[id$="--resourcePageContainerHeader-identifierLineContainer"]'
                );

                if (
                    (pluginArea && xsltCount > 10) ||
                    cpiData.currentArtifactType != "XSLT"
                ) {
                    clearInterval(interval);
                    scriptCollectionCount = 0;
                    return;
                ...
                buttons = document.getElementsByClassName(
                    "cpiHelper_pluginButton_xsltButton"
                );
                if (pluginArea && buttons.length == 0) {
                    xsltCount++;
                    buttonsForPlugins.forEach((element) => {
        ...
        if (buttonsForPlugins.length > 0) {
            ...
            var interval = setInterval(() => {
                var pluginArea = document.querySelector(
                    'span[id$="--mappingPageHeaderTitle-identifierLineContainer"]'
                );

                if (
                    (pluginArea && xsltCount > 10) ||
                    cpiData.currentArtifactType != "M_Mapping"
                ) {
                    clearInterval(interval);
                    scriptCollectionCount = 0;
                    return;
                ...
                buttons = document.getElementsByClassName(
                    "cpiHelper_pluginButton_messageMappingButton"
                );
                if (pluginArea && buttons.length == 0) {
                    xsltCount++;
                    buttonsForPlugins.forEach((element) => {
...
function dragElement(elmnt) {
    ...
    function elementDrag(e) {
        ...
        maxheight =
            window.innerHeight -
            document.getElementById("cpiHelper_contentheader").offsetHeight;
        maxwidth =
            window.innerWidth -
            document.getElementById("cpiHelper_contentheader").offsetWidth;
        // bounding position based on max top and width. making position relative in case of resize.
        let mouse_top = (elmnt.style.top =
            ((newtop < 0 || newtop >= maxheight
                ? newtop < 0
                    ? 0
                    : newtop >= maxheight
                    ? maxheight
                ...
                window.innerHeight +
            "%");
        let mouse_left = (elmnt.style.left =
            ((newleft < 0 || newleft >= maxwidth
                ? newleft < 0
                    ? 0
                    : newleft >= maxwidth
                    ? maxwidth
                ...
                chrome.storage.sync.get([name], function (result) {
                    ...
                    if (visitedIflows.length > 0) {
                        visitedIflows = visitedIflows.filter((element) => {
                            return !(
                                element.name == String(cpiArtifactId) &&
                                dataRegexp[1] == element.type
                            );
                        });
                    }
...
async function refreshPowerTrace() {
    ...
    if (timeAsStringOrNull != null && timeAsStringOrNull != undefined) {
        ...
        if (time != NaN && time < 1000 * 60 * 11) {
            ...
            if (
                btn != undefined &&
                !btn.classList.contains("cpiHelper_powertrace")
            ) {
                btn.classList.toggle("cpiHelper_powertrace");
            }
        }
...
setInterval(async function () {
    ...
    AllowedTypes = ["IFlow", "ODATA API", "REST API", "SOAP API"].includes(
        cpiData.currentArtifactType
    );
    if (
        !AllowedTypes &&
        sidebar.active &&
        !document.getElementById("__buttonxx")
    ) {
        sidebar.deactivate();
    }
    ...
    if (
        callChromeStoragePromise("CPIhelperThemeInfo") !==
        $("html").hasClass("sapUiTheme-sap_horizon")
    ) {
        await syncChromeStoragePromise(
            "CPIhelperThemeInfo",
            $("html").hasClass("sapUiTheme-sap_horizon")
        );
    }
    ...
    if (
        ...
        (nextMessageSidebarRefreshCount <= 0 ||
            (lastTabHidden > 0 && document.hidden == false))
    ) {
        ...
        nextMessageSidebarRefreshCount = calculateMessageSidebarTimerTime(
            lastTabHidden,
            lastDurationRefresh
        );
    }
