...
async function messageSidebarPluginContent(forceRender = false) {
    ...
    for (element of pluginList) {
        ...
        if (
            settings[element.id + "---isActive"] === true &&
            element?.messageSidebarContent?.onRender &&
            (!element?.messageSidebarContent?.static || forceRender == true)
        ) {
            activeness = true;
            const pluginRender = element.messageSidebarContent.onRender(
                cpiData,
                settings
            );
            if (pluginRender) {
                var div = document.getElementById(
                    "cpiHelper_messageSidebar_pluginArea_" + element.id
                );
                if (!div) {
                ...
                div.appendChild(
                    createElementFromHTML(
                        "<div class='ui tiny header'>" + element.name + "</div>"
                    )
                );
                div.appendChild(pluginRender);
                document
                    .querySelector("#cpiHelper_messageSidebar_pluginArea")
                    .appendChild(div);
            }
        }
    }
    const ctxbtnclose = document.querySelector("#cpiHelper_contentheader");
    const pluginArea = document.querySelector(
    ...
    if (ctxbtnclose.childElementCount == 2) {
        if (activeness == true) {
            ctxbtnclose.insertBefore(
                createElementFromHTML(
                    `<i id='sidebar_Plugin' class="cpiHelper_closeButton_sidebar calendar ${
                        pluginArea.classList.contains("visible")
                            ? "plus"
                            : "minus"
                    } icon"></i>`
                ),
            ...
            document
                .querySelector("#sidebar_Plugin")
                .classList.remove("cpiHelper_hidden");
            document
                .querySelector("#sidebar_Plugin")
                ...
                if (result["openSidebarOnStartup"]) {
                    twoClasssToggleSwitch(
                        document.querySelector(
                            "#cpiHelper_messageSidebar_pluginArea>.header"
                        ),
                        "cpiHelper_hidden",
                        "visible"
                    );
...
async function createPluginButtonsInMessageSidebar(runInfoElement, i, flash) {
    ...
    for (var plugin of pluginList) {
        ...
        if (settings[plugin.id + "---isActive"] === true) {
            if (
                (plugin.messageSidebarButton &&
                    !plugin.messageSidebarButton.condition) ||
                (plugin.messageSidebarButton &&
                    plugin.messageSidebarButton.condition(
                        cpiData,
                        settings,
                ...
                var button = createElementFromHTML(`<button title='${
                    plugin.messageSidebarButton.title
                }' id='cpiHelperPlugin--${plugin.id}' 
                ...
                button.onclick = async (btn) => {
                    let pluginID = btn.target.id.replace(
                        "cpiHelperPlugin--",
                        ""
                    );
                    let pluginItem = pluginList.find(
                        (element) => element.id == pluginID
                    );
                    let pluginsettings = await getPluginSettings(pluginID);
                    let pluginbtnstatus = document.querySelector(
                        `[id='cpiHelperPlugin--${pluginID}'].${runInfoElement.messageGuid}`
                    );
                    isactivebutton = !pluginbtnstatus.classList.contains(
                        "cpiHelper_plugin-active"
                    );
                    pluginItem.messageSidebarButton.onClick(
                        cpiData,
                        pluginsettings,
                        runInfoElement,
                        isactivebutton
                    ...
                    if (!isactivebutton) {
                        pluginbtnstatus.classList.remove(
                            "cpiHelper_plugin-active"
                        );
                    } else {
                        document
                            .querySelectorAll(`#outerFrame button`)
                            .forEach((e) =>
                                e.classList.remove("cpiHelper_plugin-active")
                            );
                        pluginbtnstatus.classList.add(
                            "cpiHelper_plugin-active"
                        );
                    }
...
async function createPluginButtons(type) {
    ...
    for (var plugin of pluginList) {
        ...
        if (settings[plugin.id + "---isActive"] === true) {
            if (
                (plugin[type] && !plugin[type].condition) ||
                (plugin[type] && plugin[type].condition(cpiData, settings))
            ) {
                var button = createElementFromHTML(`<button title='${
                    plugin[type].title
                }' id='cpiHelperPlugin--${
                    plugin.id
                }' class='cpiHelper_pluginButton_${type} mini ui button cpiHelper_pluginButton'>
                ${
                    plugin?.messageSidebarButton?.icon?.type === "icon"
                        ? `<span data-sap-ui-icon-content="&#${plugin.messageSidebarButton.icon.text}" class="sapUiIcon sapUiIconMirrorInRTL" style="font-family: SAP-icons; font-size: 0.9rem;"></span>`
                        : plugin[type]?.title
...
async function createPluginPopupUI(plugin) {
    ...
    container.appendChild(
        ...
        <img class="right floated mini ui image" src=${
            plugin.settings["icon"]
                ? chrome.runtime.getURL(plugin.settings["icon"].src)
                : ""
        }>
        <div class="header">${plugin.name}</div>
        <a href=${plugin.website} target="_blank" class="meta">${
            plugin.author
        }</a>
    </div>`)
    );
        ...
        if (JSON.stringify(plugin.settings) !== "{}") {
            ...
            for (var key of Object.keys(plugin.settings)) {
                if (plugin.settings[key].type == "checkbox") {
                    ...
                    checkbox.key = `${getStoragePath(
                        plugin.id,
                        key,
                        plugin.settings[key].scope
                    )}`;
                    checkbox.type = "checkbox";
                    checkbox.checked = await getStorageValue(
                        plugin.id,
                        key,
                        plugin.settings[key].scope
                    );
                ...
                if (plugin.settings[key].type == "textinput") {
                    ...
                    text.key = `${getStoragePath(
                        plugin.id,
                        key,
                        plugin.settings[key].scope
                    )}`;
                    text.placeholder = `${
                        plugin.settings[key].placeholder == undefined
                            ? plugin.settings[key].text
                            : plugin.settings[key].placeholder
                    }`;
                    text.type = "text";
                    text.value = await getStorageValue(
                        plugin.id,
                        key,
                        plugin.settings[key].scope
                    );
                    ...
                    div.appendChild(
                        createElementFromHTML(
                            `<div class="ui basic label" for="cpiHelper_popup_plugins-${plugin.id}-${key}"> ${plugin.settings[key].text}</div>`
                        )
                    );
    ...
    activeCheckbox.addEventListener("change", async function () {
        containerbox = document.querySelector(
            `#cpiHelper_popup_plugins-${plugin.id}`
        ).parentNode;
        log.log(activeCheckbox.checked);
        await syncChromeStoragePromise(
            getStoragePath(plugin.id, "isActive"),
            activeCheckbox.checked
        );
        activeCheckbox.checked
            ? containerbox.classList.add("checked")
            : containerbox.classList.remove("checked");
    ...
    div.appendChild(
        createElementFromHTML(
            `<label for="cpiHelper_popup_plugins-${plugin.id}"> Activate</label>`
        )
    );
...
function getStoragePath(pluginId, key, type = null) {
    return `${pluginId}---${
        type == "tenant" || type == "iflow" ? cpiData.tenant + "---" : ""
    }${type == "iflow" ? cpiData.integrationFlowId + "---" : ""}${key}`;
}

async function getStorageValue(pluginId, key, type = null) {
    var result = await callChromeStoragePromise(
        getStoragePath(pluginId, key, type)
    );
    if (!result) {
        return "";
    }
