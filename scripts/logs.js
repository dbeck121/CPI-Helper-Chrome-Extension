...
createLogsLeftSide = async (leftActive = false) => {
    ...
    selectorIflowStatus.innerHTML = `<option ${
        selectorIflowStatusEntry == "all" ? "selected" : ""
    } value="all">all</option><option ${
        toggleCustomOrLastEntries == "FAILED" ? "selected" : ""
    ...
    selectorCustomTop.onchange = (element) => {
        ...
        customSelection = document.getElementById(
            "cpiHelper_logs_custom_selection"
        );
    ...
    selectorCustomTop.innerHTML = `<option ${
        toggleCustomOrLastEntries == "last" ? "selected" : ""
    } value="last">last 50</option><option ${
        toggleCustomOrLastEntries != "last" ? "selected" : ""
    }>custom</option>`;
    ...
    customSelectionElement.appendChild(
        createElementFromHTML(
            `<span style="margin-top: 10px;">From:<br /></span>`
        )
    );
    customSelectionElement.appendChild(
        createElementFromHTML(
    ...
    customSelectionElement.appendChild(
        createElementFromHTML(
            `<span style="margin-top: 10px;">To:<br /></span>`
        )
    );
    ...
    list.appendChild(
        createElementFromHTML(
            `<div id="cpiHelper_log_list_for_entries" class="cpiHelper_logs_table_div"/>`
        )
    );
...
updateLogList = async () => {
    try {
        var iflowStatus = document.getElementById(
            "cpiHelper_logs_iflow_status"
        ).value;
        var startDate = document.getElementById(
            "cpiHelper_logs_start_date"
        ).value;
        var endDate = document.getElementById("cpiHelper_logs_end_date").value;
        var startTime = document.getElementById(
            "cpiHelper_logs_start_time"
        ).value;
        var endTime = document.getElementById("cpiHelper_logs_end_time").value;
        var artifact = document.getElementById(
            "logs-left-side_cpiHelper_artifactList"
        ).value;
        var dateType = document.getElementById(
            "cpiHelper_logs_date_type"
        ).value;
        var listPlace = document.getElementById(
            "cpiHelper_log_list_for_entries"
        );
        var startDateTimeInUTC = new Date(
            startDate + "T" + startTime + ":00.000"
        )
            .toISOString()
            .replace("Z", "");
        var endDateTimeInUTC = new Date(endDate + "T" + endTime + ":00.000")
            .toISOString()
            .replace("Z", "");
        listPlace.innerHTML = "";
        ...
        if (response.length == 0) {
            ...
            element.innerHTML =
                '<td style="padding:0px;" colspan="2">No logs found</td>';
        ...
        if (response.length == 40) {
            let element = document.createElement("tr");
            element.innerHTML =
                '<td style="padding:0px;" colspan="2">last 40 logs in period</td>';
            list.appendChild(element);
        }
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            if (date.substr(0, 10) != lastDay) {
                ...
                day.innerHTML = `<td style="padding:0px;" colspan="2">${date.substr(
                    0,
                    10
                )}</td>`;
            ...
            button.onclick = (event) => {
                if (
                    ...
                    document
                        ...
                        .classList.remove("cpiHelper_logs_selected_button");
                }
...
updateArtifactList = async () => {
    ...
    if (list) {
        var response = await makeCallPromise(
            ...
            "/" +
                cpiData.urlExtension +
                "Operations/com.sap.it.op.tmn.commands.dashboard.webui.KnownArtifactsListCommand",
            false
        );
        log.log("response");
        responseList = new XmlToJson().parse(response)[
            "com.sap.it.op.tmn.commands.dashboard.webui.KnownArtifactsListResponse"
        ]["knownArtifacts"];
        list.innerHTML = "";
...
createLogsRightSide = async (runId, leftActive = false) => {
    ...
    if (runId) {
        ...
        logs.innerHTML =
            '<div class="cpiHelper_tabs">Please choose a message to show logs. You can also click the speech bubble icon in Messages Sidebar.</div>';
    }
...
updateRightSide = async (runId) => {
    ...
    if (rightSide) {
        ...
        page.children[1].innerHTML =
            '<div class="cpiHelper_tabs">Please wait... The request is being processed. This might take some time.</div>';
...
createPersistLogsContent = async (messageId) => {
    var innerpersist = async (input) => {
                ...
                content: async (input) => {
                    return formatTrace(
                        await makeCallPromise(
                            ...
                            "/" +
                                cpiData.urlExtension +
                                "odata/api/v1/MessageStoreEntries('" +
                                input.item +
                                "')/$value",
                            false
                        ),
        ...
        return await createTabHTML(
            persistTabs,
            "cpiHelper_persistLogs_tabs" + input.count
        );
    };

    entriesList = JSON.parse(
        await makeCallPromise(
            "GET",
            "/" +
                cpiData.urlExtension +
                ...
                messageId +
                "')/MessageStoreEntries?$format=json",
            false
        )
    );
    log.log(entriesList);
    ...
    for (item of entriesList.d.results.sort(function (a, b) {
        return a.MessageStoreId.toLowerCase() > b.MessageStoreId.toLowerCase()
            ? 1
            : -1;
    })) {
        tabs.push({
            ...
            content:
                count == 0
                    ? await innerpersist({ item: item.Id, count: count })
                    : innerpersist,
            item: item.Id,
            active,
            count
...
createLogsInfo = async (messageId) => {
    try {
        ...
        showToast(
            "Check input data.",
            "Error while fetching logs. Message ID not found.",
            "error"
        );
    ...
    logStart.setTime(
        logStart.getTime() - logStart.getTimezoneOffset() * 60 * 1000
    );
    ...
    if (input.LogEnd) {
        ...
        logEnd.setTime(
            logEnd.getTime() - logEnd.getTimezoneOffset() * 60 * 1000
        );
        valueList.push({
            Name: "End Time",
    ...
    valueList.forEach((item) => {
        result += `<tr class="${item.Type == "header" ? "blue" : ""}"><td>${
            item.Name
        }</td><td style="word-break: break-all;">${htmlEscape(
            item.Value
        )}</td></tr>`;
    });
    result += "</tbody></table>";
            ...
            content: async (input) => {
                return formatTrace(
                    await makeCallPromise(
                        ...
                        "/" +
                            cpiData.urlExtension +
                            "odata/api/v1/MessageProcessingLogAttachments('" +
                            input.item +
                            "')/$value",
                        false
                    ),
                    "cpiHelper_runLogsItem" + input.item
                );
            },
            item: item.Id,
            active
