...
async function features_home() {
    ...
    features.forEach(
        (it, index) =>
            (feature.innerHTML += `<div class="col mb-5"><div class="feature ${
                colors[index % colors.length]
            } bg-gradient text-white rounded-3 mb-3"><i class="fas ${
                collection[index % collection.length]
            }"></i></div>
        <h2 class="h5">${it.title}</h2><p class="mb-0">${it.desc}</p></div>`)
    );
    $("#home > header > div > div img").attr(
        "src",
        baseUrl + "/images/undraw_in_sync_re_jlqd.svg"
    );
...
async function users_details() {
    contributors.forEach((it) => {
        ...
        div.innerHTML = `<div class="text-center"><a href="https://github.com/${
            it.username
        }">
        <img class="img-fluid rounded-circle mb-4 px-4" src="https://github.com/${
            it.username
        }.png" alt="..." /></a>
        <h5 class="fw-bolder">${it.name.replace(/\b\w/g, (c) =>
            c.toUpperCase()
        )}</h5>
