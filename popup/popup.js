...
async function addLastVisitedIflows() {
    ...
    var compact =
        document
            .querySelector("#cpi_compact_mode>.active")
    ...
    var html = `<div class="ui horizontal divider header">Last Visited on Tenant ${
        name.split("_")[1]
    }</div>`;
    if (compact) {
...
function getSideBarAlwaysVisible() {
    ...
    chrome.storage.sync.get(["openMessageSidebarOnStartup"], function (result) {
        document
            .querySelectorAll("#openMessageSidebarOnStartup>.button")
            [result["openMessageSidebarOnStartup"] ? 0 : 1].classList.add(
                "active"
            );
        document
            .querySelector("#openMessageSidebarOnStartup")
            .addEventListener("click", () => {
                ...
                let ctnx =
                    document
                        .querySelector("#openMessageSidebarOnStartup>.active")
                        .getAttribute("data") === "true";
    ...
    chrome.storage.sync.get(["openSidebarOnStartup"], function (result) {
        document
            .querySelectorAll("#openSidebarOnStartup>.button")
            [result["openSidebarOnStartup"] ? 0 : 1].classList.add("active");
        document
            .querySelector("#openSidebarOnStartup")
            .addEventListener("click", () => {
                document
                    .querySelectorAll("#openSidebarOnStartup>.button")
                    .forEach((e) => e.classList.toggle("active"));
                let ctnx =
                    document
                        .querySelector("#openSidebarOnStartup>.active")
                        .getAttribute("data") === "true";
...
function addTenantSettings() {
    const cpi_compact_mode =
        localStorage.getItem("modecpi_compact_mode") !== "true"; //deafult cozy
    ...
    <div>
        ...
        <div id="cpi_compact_mode" class="ui label buttons">
            ...
            <div data=true class="ui toggle basic ${
                !cpi_compact_mode ? "active" : ""
            } button">Compact</div>
            <div data=false class="ui toggle basic ${
                cpi_compact_mode ? "active" : ""
            } button">Cozy</div>
        </div><br/> 
    ...
    document
        .querySelector("#one > i")
        .classList.add(cpi_compact_mode ? "expand" : "compress");
    document
        .querySelector(
            "#tenantSettings > div >div > .buttons > button:nth-child(1)"
        )
        .addEventListener("click", () => inputReset("iflow"));
    document
        .querySelector(
            "#tenantSettings > div >div > .buttons > button:nth-child(2)"
        )
        .addEventListener("click", () => inputReset("reset"));
    //Default zoom
    document.body.style.zoom = cpi_zoom_level ? `${cpi_zoom_level}%` : 85 + "%";
    document.querySelector("#setzoom input").value = cpi_zoom_level
        ? cpi_zoom_level
        : 85;
    document.querySelector("#setzoom button").addEventListener("click", () => {
        document.querySelector("#setzoom input").value = 85;
        document
            .querySelector("#setzoom input")
            .dispatchEvent(new Event("change"));
    });
    document.querySelector("#setzoom input").addEventListener("change", () => {
        const zoom = document.querySelector("#setzoom input");
        if (
            parseInt(zoom.max) >= parseInt(zoom.value) &&
            parseInt(zoom.value) >= parseInt(zoom.min)
        ) {
    ...
    document.querySelector("#cpi_help_mode").addEventListener("click", () => {
        document
            .querySelectorAll("#cpi_help_mode>.button")
            .forEach((e) => e.classList.toggle("active"));
        localStorage.setItem(
            "cpi_help_mode",
            document
                .querySelector("#cpi_help_mode>.active")
                .getAttribute("data") === "true"
        );
        document
            .querySelector(
                "#tenantSettings > div:nth-child(4) > div.ui.segment"
            )
            .classList.toggle("hidden"); //#tenantSettings>div>div:has(table)
    });
    //reset color btn
    document
        .querySelector("#tenantSettings > div >div > button")
        .addEventListener("click", async () => {
            const tenantColor = document.querySelector("#colorSelect");
            tenantColor.value = (await callChromeStoragePromise(
                "CPIhelperThemeInfo"
            ))
                ? "#ffffff"
                : "#354a5f";
    ...
    document
        .querySelector("#cpi_compact_mode")
        .addEventListener("click", () => {
            document
                .querySelectorAll("#cpi_compact_mode>.button")
                .forEach((e) => e.classList.toggle("active"));
            localStorage.setItem(
                "modecpi_compact_mode",
                document
                    .querySelector("#cpi_compact_mode>.active")
                    .getAttribute("data") === "true"
            );
    ...
    chrome.storage.local.get(
        ["cpi_top_mode"],
        (result) =>
            (document.querySelector("#cpi_top_mode input").value =
                result.cpi_top_mode ? result.cpi_top_mode : 300)
    );
    document
        .querySelector("#cpi_top_mode input")
        .addEventListener("change", () => {
            chrome.storage.local.set({
                cpi_top_mode: document.querySelector("#cpi_top_mode input")
                    .value
            });
        });
}
...
function addTenantUrls() {
    ...
    tenantUrls.innerHTML = `<div class="ui horizontal divider header">Main Links</div>
                <div class="ui wrapping buttons fluid">
                    <a class="ui l-green button" target="_blank" href="${
                        host + "/shell/monitoring/Messages/"
                    }">Processed Messages</a>
                    <a class="ui l-red button" target="_blank" href="${
                        host +
                        "/shell/monitoring/Messages/%7B%22status%22%3A%22FAILED%22%2C%22time%22%3A%22PASTHOUR%22%2C%22type%22%3A%22INTEGRATION_FLOW%22%7D"
                    }">Failed Messages</a>
                    <a class="ui l-black button" target="_blank" href="${
                        host + "/shell/monitoring/Artifacts/"
                    }">Integration Content</a>
                    <a class="ui l-blue button" target="_blank" href="${
                        host + "/shell/design"
                    }">Design</a> 
                </div>
                ...
                <div class='ui menu'>
                    <a class="ui item" href="${
                        host + "/shell/monitoring/Overview"
                    }" target="_blank">
                    ...
                    <div class="ui wrapped wrapping buttons fluid" style="display: flex;flex-direction: column;">
                        <div class="three ui buttons">
                            <a class="ui button" href="${
                                host + "/shell/monitoring/SecurityMaterials"
                            }" target="_blank">Security Material</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/Keystore"
                            }" target="_blank">Keystore</a>
                            <a class="ui button" href="${
                                host +
                                "/shell/monitoring/CertificateUserMappings"
                            }" target="_blank">Certificate User Mappings</a>
                        </div>
                        <div class="three ui buttons">
                            <a class="ui button" href="${
                                host + "/shell/monitoring/AccessPolicies"
                            }" target="_blank">Access Policies</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/JdbcMaterial"
                            }" target="_blank">JDBC Material</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/Connectivity"
                            }" target="_blank">Connectivity Tests</a>
                        </div>
                        <div class="ui fitted divider"></div>
                        <div class="three ui buttons">
                            <a class="ui button" href="${
                                host + "/shell/monitoring/DataStores"
                            }" target="_blank">Data Stores</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/Variables"
                            }" target="_blank">Variables</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/MessageQueues"
                            }" target="_blank">Message Queues</a>
                        </div>
                        <div class="three ui buttons">
                            <a class="fluid ui button" href="${
                                host + "/shell/monitoring/NumberRangeObject"
                            }" target="_blank">Number Ranges</a>
                            <a class="fluid ui button" href="${
                                host + "/shell/monitoring/UserRoles"
                            }" target="_blank">User Roles</a>
                            <a class="fluid ui button" href="${
                                host + "/shell/monitoring/MessageUsage"
                            }" target="_blank">Message Usage</a>                            
                        </div>
                        <div class="ui fitted divider"></div>
                        <div class="three ui buttons">
                            <a class="ui button" href="${
                                host + "/shell/monitoring/SystemLogs"
                            }" target="_blank">System Logs</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/Locks"
                            }" target="_blank">Message Locks</a>
                            <a class="ui button" href="${
                                host + "/shell/monitoring/DesigntimeLocks"
                            }" target="_blank">Designtime Artifact Locks</a>
                        </div>
                    </div>
                </div>
                <div class='ui menu'>
                    <a class="ui item" href="${
                        host + "/shell/settings"
                    }" target="_blank"><strong>API Management</strong></a>
                    <div class="ui wrapped wrapping buttons fluid">
                        <a class="ui button" href="${
                            host + "/shell/configure"
                        }" target="_blank">Configure APIs</a>
                        <a class="ui button" href="${
                            host + "/shell/testconsole"
                        }" target="_blank">Test APIs</a>
                        <a class="ui button" href="${
                            host + "/shell/analytics"
                        }" target="_blank">Analyze APIs</a> 
                    </div>
                </div>
                <div class='ui menu'>
                    <a class="ui item " href="${
                        host + "/shell/tpm/companyProfile"
                    }" target="_blank"><strong>Trading Partner Management</strong></a>
                    <div class="ui wrapped wrapping buttons fluid">
                        <a class="ui button" href="${
                            host + "/shell/b2bmonitor/landing"
                        }" target="_blank">B2B Monitor</a>
                        <a class="ui button" href="${
                            host + "/shell/tpm/companyProfile"
                        }" target="_blank">Company Profile</a>
                        <a class="ui button" href="${
                            host + "/shell/tpm/agreementTemplates"
                        }" target="_blank">Agreement Templates</a>
                        <a class="ui button" href="${
                            host + "/shell/tpm/agreements"
                        }" target="_blank">Agreements</a>                                                
                        <a class="ui button" href="${
                            host + "/shell/tpm/tradingPartners"
                        }" target="_blank">Trading Partners</a>                                                
                        <a class="ui button" href="${
                            host + "/shell/tpm/pdContent"
                        }" target="_blank">Partner Directory Data</a>
                        <a class="ui button" href="${
                            host + "/shell/tpm/crossActions"
                        }" target="_blank">Cross Actions</a>                        
                    </div>
                </div>
                <div class='ui menu'>
                    <a class="ui item " href="${
                        host + "/shell/tpm/companyProfile"
                    }" target="_blank"><strong>Integration Advisor</strong></a>
                    <div class="ui wrapped wrapping buttons fluid">
                        <a class="ui button" href="${
                            host + "/shell/migs"
                        }" target="_blank">MIGs (Message Implementation Guidelines)</a>
                        <a class="ui button" href="${
                            host + "/shell/mags"
                        }" target="_blank">MAGs (Mapping Guidelines)</a>
                        <a class="ui button" href="${
                            host + "/shell/customtypesystems"
                        }" target="_blank">Custom Type Systems</a>
                    </div>
                </div>`;
}
...
function tenantIdentityChanges() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.tabs.sendMessage(tab.id, "get", (response) => {
            ...
            if (response) {
                ...
                chrome.storage.sync.get("CPIhelperThemeInfo", (theme) => {
                    ...
                    $("html").attr(
                        "class",
                        isDarkmode ? "ch_dark" : "ch_light"
                    );
                    tenantColor.value = hostData.color = adjustColorLimiter(
                        tenantColor.value,
                        isDarkmode ? 80 : 20,
                        25,
                        !theme["CPIhelperThemeInfo"]
                    );
                    tenantColor.dispatchEvent(new Event("change"));
                    console.log(tenantColor.value);
                    popupcolor.style.setProperty(
                        "--cpi-text-color",
        ...
        tenantColor.addEventListener("change", async () => {
            ...
            // }
            tenantColor.value = adjustColorLimiter(
                tenantColor.value,
                !theme ? 80 : 20,
                25,
                !theme
...
function adjustColorLimiter(ihex, limit, dim, abovelimit = false) {
    ...
    l = Math.max(
        0,
        Math.min(
            l > limit === abovelimit ? l + dim * (abovelimit ? -1 : 1) : l,
            100
        )
    );
    ohex = hslToHex(h, s, l);
